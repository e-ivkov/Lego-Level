...
public class GridRecognizer: IRecognizer<Color>
{
	...
	public List<Item<Color>> Recognize(Texture2D image)
	{
		...
		List<Item<Color>> blocks = new List<Item<Color>> ();
		for (int i = 0; i < averageColors.GetLength(0); i++) {
			for (int j = 0; j < averageColors.GetLength(1); j++) {
				averageColors[i, j] /= nPixels;
				blocks.Add(new Item<Color>(new Vector2(i,j), GridColors.OrderBy(color => Vector4.Distance(averageColors[i, j], color)).ToArray()[0]));
			}
		}
		return blocks;
	}
